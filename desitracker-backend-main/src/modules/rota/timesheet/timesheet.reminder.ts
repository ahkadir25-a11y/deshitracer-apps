...
export async function runForgotClockOutReminder(): Promise<{
  checked: number;
  emailed: number;
  failed: number;
}> {
  ...
  const candidates = await RotaTimesheet.find({
    clockOut: null,
    isDeleted: false,
    reminderSentAt: null,
    shift: { $ne: null },
  })
    .populate<{ shift: { _id: any; endAt: Date } }>('shift', 'endAt')
    .populate<{ employee: { _id: any; firstName: string; email: string } }>(
      'employee',
      'firstName email',
    )
    .lean({ virtuals: false });
  ...
  for (const ts of candidates) {
    const shift = (ts as any).shift;
    const employee = (ts as any).employee;
    if (!shift?.endAt || !employee?.email) continue;

    const shiftEnd = new Date(shift.endAt);
    if (shiftEnd.getTime() > cutoff.getTime()) continue;

    // Skip if overtime is running — they're still working intentionally.
    const otStatus = (ts as any).overtime?.status;
    if (otStatus === 'RUNNING') continue;
    ...
    try {
      ...
      const empDoc = await RotaEmployee.findById((ts as any).employee).select('expoPushToken').lean();
      ...
      if (pushToken) {
        await sendExpoPush({
          ...
          sound: 'default',
          data: { type: 'FORGOT_CLOCKOUT', timesheetId: String((ts as any)._id) },
        });
      }
      ...
      failed += 1;
      console.error(
        `[forgotClockOut] failed for timesheet ${(ts as any)._id}: ${err?.message}`,
      );
    }
  }
...
export async function runAutomaticOvertimeTransition(): Promise<{
  checked: number;
  transitioned: number;
  failed: number;
}> {
  ...
  const candidates = await RotaTimesheet.find({
    ...
    .populate<{ shift: { _id: any; endAt: Date; ownerMandatedOvertime: boolean } }>('shift', 'endAt ownerMandatedOvertime')
    .lean({ virtuals: false });
  ...
  for (const ts of candidates) {
    const shift = (ts as any).shift;
    if (!shift?.endAt || !shift.ownerMandatedOvertime) continue;
    ...
    if (now.getTime() > shiftEnd.getTime()) {
      try {
        ...
        failed += 1;
        console.error(`[runAutomaticOvertimeTransition] failed for timesheet ${(ts as any)._id}: ${err?.message}`);
      }
    }
  }
...
export async function runAutoClockOut(): Promise<{
  ...
  const candidates = await RotaTimesheet.find({
    ...
    .populate<{ shift: { _id: any; endAt: Date; ownerMandatedOvertime: boolean } }>(
      'shift',
      'endAt ownerMandatedOvertime',
    )
    .lean({ virtuals: false });
  ...
  for (const ts of candidates) {
    ...
    if (shift.ownerMandatedOvertime) continue;
    ...
    try {
      ...
      failed += 1;
      console.error(
        `[autoClockOut] failed for timesheet ${(ts as any)._id}: ${err?.message}`,
      );
    }
  }
