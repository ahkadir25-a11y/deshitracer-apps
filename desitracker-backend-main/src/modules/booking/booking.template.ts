...
const formatWhen = (d: Date | string): string => {
...
export const bookingConfirmationTemplate = (data: BookingConfirmationData): string => {
...
// Notification sent to the BUSINESS (owner/staff) when a new reservation comes
// in, so they're alerted even if they don't have the app open. Includes the
// customer's contact details so the business can reach them if needed.
interface BookingOwnerData {
  businessName: string;
  name: string;
  phone?: string;
  email?: string;
...
export const bookingOwnerNotificationTemplate = (data: BookingOwnerData): string => {
